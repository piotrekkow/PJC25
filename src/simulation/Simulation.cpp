...
void Simulation::initialize()
{
	...
	auto wlin = network_->addLink({ 100, 510 }, { 700, 510 }, ie, i);
	auto wlout = network_->addLink({ 700, 490 }, { 100, 490 }, i, ie);
	wlin->addLane();
	wlin->addLane();
	wlout->addLane();
	auto elin = network_->addLink({ 1800, 490 }, { 900, 490 }, iw, i);
	auto elout = network_->addLink({ 900, 510 }, { 1800, 510 }, i, iw);
	elin->addLane();
	elin->addLane();
	elout->addLane();
	auto nlin = network_->addLink({ 790, 100 }, { 790, 400 }, in, i);
	auto nlout = network_->addLink({ 810, 400 }, { 810, 100 }, i, in);
	nlin->addLane();
	auto slin = network_->addLink({ 810, 1000 }, { 810, 600 }, is, i);
	slin->addLane();
	slin->addLane();
	auto slout = network_->addLink({ 790, 600 }, { 790, 1000 }, i, is);
	slout->addLane();
